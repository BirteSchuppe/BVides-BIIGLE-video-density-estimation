...
sx <- smooth.spline(t, x, cv = TRUE)
sy <- smooth.spline(t, y, cv = TRUE)
sz <- smooth.spline(t, z, cv = TRUE)


gam_east_gcv <- gam(x ~ s(t, bs = "ps",k=100),  method="REML")
gam_north_gcv <- gam(y ~ s(t, bs = "ps",k=100), method="REML")
gam_depth_gcv <- gam(z ~ s(t, bs = "ps",k=100), method="REML") 
...
# or if using gam
summary(gam_east_gcv) %>% print()
summary(gam_north_gcv) %>% print()
summary(gam_depth_gcv) %>% print()
...
# if using gam
interpolated_smoothed_navigation %>% 
 mutate(LON3 = predict(gam_east_gcv, newdata = data.frame(t = t2) ) %>% as.vector() ,
        LAT3 = predict(gam_north_gcv, newdata = data.frame(t = t2) ) %>% as.vector(), 
        DEPTH3 = predict(gam_depth_gcv, newdata = data.frame(t = t2) ) %>% as.vector()         
 ) -> interpolated_smoothed_navigation
...
plot3d <- plot_ly() %>%
  ...
  # use the smoothed XYZ
  add_trace(x = interpolated_smoothed_navigation$LON2, y = interpolated_smoothed_navigation$LAT2, z = - interpolated_smoothed_navigation$DEPTH2, mode = "lines", type = "scatter3d", name = "smoothed with cubic splined ") %>%
    add_trace(x = interpolated_smoothed_navigation$LON2, y = interpolated_smoothed_navigation$LAT2, z = - interpolated_smoothed_navigation$DEPTH2, mode = "markers", type = "scatter3d", name = "smoothed with cubic splined ",
               marker = list(size = 3) ) %>%
...
saveWidget(plot3d, paste0( "~/temp/", "3D Navigation ", ".html") )
...
interpolated_smoothed_navigation <- interpolated_smoothed_navigation %>%
  mutate(DEPTH3_lower = ( pred_depth$fit - 1.96 * pred_depth$se.fit) %>% as.vector(),
         DEPTH3_upper =  ( pred_depth$fit + 1.96 * pred_depth$se.fit) %>% as.vector() ) %>% 
    mutate(LON3_lower = ( pred_x$fit - 1.96 * pred_x$se.fit) %>% as.vector(),
         LON3_upper =  ( pred_x$fit + 1.96 * pred_x$se.fit) %>% as.vector() )  %>% 
  mutate(LAT3_lower = ( pred_y$fit - 1.96 * pred_y$se.fit) %>% as.vector(),
         LAT3_upper =  ( pred_y$fit + 1.96 * pred_y$se.fit) %>% as.vector() )  %>% 
  as_tibble()


interpolated_smoothed_navigation %>% 
  select(datetime, DEPTH3, DEPTH3_lower, DEPTH3_upper) %>% 
  glimpse()

interpolated_smoothed_navigation %>% 
  ggplot(aes(x = datetime)) +
  geom_ribbon(aes(ymin = DEPTH3_lower, ymax = DEPTH3_upper), fill = "lightblue", alpha = 0.5) +
  geom_line(aes(y = DEPTH3), color = "blue") +
...
plot_depth_ci <- plot_ly(data = interpolated_smoothed_navigation, x = ~datetime) %>%
  add_lines(y = ~DEPTH3, name = "Predicted Depth (GAM)", line = list(color = 'blue')) %>%
  add_ribbons(ymin = ~DEPTH3_lower, ymax = ~DEPTH3_upper, name = "95% CI",
...
interpolated_smoothed_navigation %>% 
  ggplot(aes(x = datetime)) +
  geom_ribbon(aes(ymin = LON3_lower, ymax = LON3_upper), fill = "lightgreen", alpha = 0.5) +
  geom_line(aes(y = LON3), color = "green") +
...
interpolated_smoothed_navigation %>% 
  ggplot(aes(x = datetime)) +
  geom_ribbon(aes(ymin = LAT3_lower, ymax = LAT3_upper), fill = "lightcoral", alpha = 0.5) +
  geom_line(aes(y = LAT3), color = "red") +
