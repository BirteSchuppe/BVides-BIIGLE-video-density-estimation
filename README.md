...
To gain consistent frequency of navigation data down to 1 second, the smoothed GAMs were used to predict latitude, longitude and depth. Splines have been used for smoothing noisy GPS data and P-splines are useful for sparse data, making them applicable for unevenly and/or less frequent pinged navigation data. The amount of smoothness in BVides is data-driven by the smoothness selection criterion of REML. Constraining the upper limit of the degrees of freedom (basis dimension k=175) was deemed satisfactory large enough to follow the underlying wigglyness of the trajectory but preventing oversmoothing. The smoothing model fit (choice of appropriate basis dimension) is provided by computation of effective degrees of freedom value (EDF) and k-index inside the gam.check function.
